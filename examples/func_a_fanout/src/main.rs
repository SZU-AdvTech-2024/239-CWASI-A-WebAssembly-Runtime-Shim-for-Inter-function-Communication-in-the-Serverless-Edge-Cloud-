...
/*
    docker build -t funca_fanout . && docker save -o funca_fanout.tar funca_fanout:latest && sudo ctr -n mysp images rm docker.io/library/funca_fanout:latest && \
    sudo ctr -n mysp images import funca_fanout.tar
...
pub async fn process_response(file: String, funcb_url:String) -> Duration {
    let url_str = format!("http://{}/hello",&funcb_url);
