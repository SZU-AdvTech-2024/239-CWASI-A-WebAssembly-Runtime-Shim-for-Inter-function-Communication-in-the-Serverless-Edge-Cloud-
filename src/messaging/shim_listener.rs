...
impl ShimListener {
    ...
    pub fn read_from_memory(&mut self){
        ...
        loop {
            ...
            if shm_fd >= 0 {
                ...
                // println!("try to reopen share mem");
            }
        }
    ...
    fn call_vm_without_input(&mut self) -> Result<i64, Box<dyn std::error::Error>>{
...
pub unsafe fn send_shared_memory(message:Vec<u8>, fn_name: &str) -> Result<String, Error> {

    // Create shared memory
    let shim_name ="/func_b".to_string();
    let shim_name = fn_name.split('.').nth(0).unwrap().to_string();
...
pub async fn init_listener(bundle_path: String, oci_spec: Spec, vm: Vm) -> Result<(), Box<dyn std::error::Error>>{
    ...
    // loop {
    //     let input = connect_to_source("127.0.0.1:8080".to_string())?;
    //     listener.call_vm_with_input(input).expect("TODO: panic message");
        
    // }

    // listener.call_vm_without_input().expect("call vm without input error");
