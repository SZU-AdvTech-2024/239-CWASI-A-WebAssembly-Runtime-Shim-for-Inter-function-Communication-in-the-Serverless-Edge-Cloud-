...
use containerd_shim::protos::events::image;
...
/*
sudo ctr -n k8s.io run --rm --runtime=io.containerd.cwasi.v1 --annotation cwasi.secondary.function=false --net-host=true --env STORAGE_IP=127.0.0.1:9999 --env REDIS_IP=127.0.0.1 --env FUNCTIONS_NUM=1 docker.io/keniack/func_a:latest funcaaaaa /func_a.wasm /func_b.wasm file_1M.txt
 */
pub fn prepare_module(mut vm: Vm, spec: &oci::Spec, stdin_path: String, stdout_path: String, stderr_path: String ) -> Result<Vm, WasmRuntimeError> {
    ...
    println!("opening rootfs");
    let rootfs_path = oci::get_root(spec).to_str().unwrap();
    let root = format!("/:{}", rootfs_path);
    let mut preopens = vec![root.as_str()];

    println!("opening mounts");
    let mut mounts = oci_utils::get_wasm_mounts(spec);
    println!("mounts: {:#?}", mounts);
    // let mut modified_mounts: Vec<String> = mounts.iter()
    // .map(|s| format!("{}:{}", s, s))
    // .collect();
    // let mut result = modified_mounts.iter().map(|s| {s.as_str()}).collect();
    preopens.append(&mut mounts);
    // preopens.retain(|&x| x != "/etc/hosts");
    // preopens.retain(|&x| x != "/etc/resolv.conf");
    ...
    println!("register wasm app from file {:?}",mod_path);
    let mod_path_print = mod_path.clone();

    // let additional_modules = extract_modules_from_wat(mod_path_print.as_path(), args[1]);
    // let image_names = args[1].split_whitespace().map(|s| s.to_string()).collect();
    // println!("image_names: {:#?}", image_names);
    // let additional_modules: Vec<String> = snapshot_utils::get_existing_image(image_names);
    // println!("additional_modules: {:#?}", additional_modules);
    ...
    //     println!("register_module_from_file, module_name: {:#?}, additional_module{:#?}", module_name, additional_module);
    ...
    println!("================================================================================================");
    println!("preopens: {:#?}", preopens);
    // [error] Bind guest directory failed:54
